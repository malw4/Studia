...
public class HexagonalNeighbour extends NeighbourAlgorithm {

    enum HexagonalType {
        slash, backslash, random
    }

    public HexagonalType direction;

    public HexagonalNeighbour(int w, int h, boolean isPeriod) {
        width = w;
        height = h;
        isPeriodic = isPeriod;
        direction = HexagonalType.random;
    }

    public void setType(int num) {
        direction = HexagonalType.values()[num];
    }

    @Override
    protected Color decideCell(int x, int y) {
        Color[] neighbours = new Color[6];
        int dir = (direction.ordinal() < 2) ? direction.ordinal() : new Random().nextInt(2);
        int j = 0;
        switch (dir) {
            case 0: // slash
                if (isPeriodic) {
        ...
        switch (dir) {
            ...
            case 1: // backslash
                if (isPeriodic) {
                    ...
                    if ((x - 1 >= 0) && isEdge(x - 1, y ))
                        energy++;
                    if ((y + 1 < height) && (x - 1 >= 0) && isEdge(x - 1, y + 1 ))
                        energy++;
                }
                break;
        }
            ...
            case 0: // slash
                if (isPeriodic) {
                    ...
                    if ((y + 1 < height) && !thisColor.equals(colorsTab[y + 1][x]))
                        return true;
                    if ((x + 1 < width) && (y + 1 < height) && !thisColor.equals(colorsTab[y + 1][x + 1]))
                        return true;
